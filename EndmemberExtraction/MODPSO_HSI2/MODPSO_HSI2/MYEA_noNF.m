   ...
   recordfit(2,1)=mean(f_x(F0,2));
    ...
    for i=1:maxiter
        ...
        for j =1:pop_num  
            ...
            a1=TournamentSelection((pop_num*2)/3,1,f_x(:,1),f_x(:,2));
            a2=TournamentSelection(10,1,f_x(:,1),f_x(:,2));
...
%         drawnow;       
   ...
   time4_noNF=etime(TIME2,TIME1);
%  time4 = cputime - T;              %runtime
 recordfit4_noNF=recordfit;
        s=new_f_x(F(1,:)==1,:);
%         subplot(2,4,1); 
        scatter(s(:,1),s(:,2));
%         legend('MY')        
...
Mbest_x_noNF=best_x;
Ms_noNf=s;
Mrecord_SAM_noNF =record_SAM; 
save MYEA_data_noNF Mbest_x_noNF Ms_noNF Mrecord_SAM_noNF  time4_noNF recordfit4_noNF
