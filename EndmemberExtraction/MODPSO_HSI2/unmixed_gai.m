...
function [rmse,risk]=unmixed_gai(mixed_pixel,endmember,unmixed_method)
%丰度反演
%输入：
%   mixed_pixel 混合像元矩阵
%   endmember 端元向量组
...
for i = 1:endmember_n
    for j = 1:endmember_n
        cov = sum((endmember(:,i)-mixed_pixel(:,1)).*(endmember(:,j)-mixed_pixel(:,1)))/band_n;
        risk = sum(abundance(i,:).*abundance(j,:),2)*cov;
    end
end
