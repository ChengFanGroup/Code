...
%       MaxFront,   排序的最大前面编号
    ...
    elseif strcmp(Operation,'first')
        Kind = 3;
    elseif strcmp(Operation,'new')
        Kind = 4;
    elseif strcmp(Operation,'new1')
        Kind = 5;   
    elseif strcmp(Operation,'new2')
        Kind = 6;  
    else
        Kind = 1;
    end
    ...
    while (Kind==1 && sum(cz)<50) || (Kind==2 && sum(cz)<N/2) || (Kind==3 && MaxFront<1)||(Kind==4&&sum(cz)<20)||(Kind==5&&sum(cz)<N)||(Kind==6&&sum(cz)<25)
        MaxFront = MaxFront+1;
        d = cz;
        for i = 1 : N
            if ~d(i)
