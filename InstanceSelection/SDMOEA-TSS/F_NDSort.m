...
%   Copyright 2015 Ye Tian
    ...
    [~,kind] = ismember(Operation,{'all','half','bin2','bin3', 'bin4','bin5','bin6'});
    [N,M]    = size(PopuObj);
    FrontNO  = inf(1,N);
    MaxFNO   = 0;
    [PopuObj,rank] = sortrows(PopuObj);
    while (kind==1 && sum(FrontNO<inf)<40) || (kind==2 && sum(FrontNO<inf)<N/2)||(kind==3 && sum(FrontNO<inf)<50) || (kind==4 && sum(FrontNO<inf)<33)||(kind==5&&sum(FrontNO<inf)<25||kind==6&&sum(FrontNO<inf)<20||kind==7&&sum(FrontNO<inf)<17)
        MaxFNO = MaxFNO + 1;
        for i = 1 : N
            if FrontNO(i) == inf
                Dominated = false;
